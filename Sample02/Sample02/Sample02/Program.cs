...
namespace Sample02
{
    class Program
    {
        private static readonly string connStr = "Data Source=.;User ID=sa;Password=123456;Initial Catalog=CORE.CMS;Pooling=true;Max Pool Size=100;";
...
VALUES   (@title,@content,@status,@add_time,@modify_time)";
        ...
        static void test_select_one()
        {
            using (var conn = new SqlConnection(connStr))
            {
                ...
                var result = conn.QueryFirstOrDefault<Content>(sql_insert, new { id = 3 });
        ...
        static void test_select_list()
        {
            using (var conn = new SqlConnection(connStr))
            {
                ...
                var result = conn.Query<Content>(sql_insert, new { ids = new int[] { 1, 3 } });
                Console.WriteLine($"test_select_one：查到的数据为：");
            }
        }
